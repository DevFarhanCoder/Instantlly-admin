...
function DashboardContent() {
  ...
  const fetchStats = async () => {
    try {
      setLoadingMessage(
        "Waking up server (Render free tier may take up to 60 seconds)...",
      );
  ...
  const fetchUsers = async () => {
    ...
    try {
      ...
      console.log(
        ...
        data.users?.length,
        "First user credits:",
        data.users?.[0]?.stats?.credits,
        "Last user credits:",
        data.users?.[data.users?.length - 1]?.stats?.credits,
      );
  ...
  const exportPhoneNumbers = async () => {
    try {
      const response = await axios.get(
        `${API_BASE}/api/admin/users/export-phones`,
        {
          headers: { "x-admin-key": ADMIN_KEY },
          responseType: "blob",
          timeout: 30000,
  ...
  const exportUserContacts = async (userId: string, userName: string) => {
    try {
      const response = await axios.get(
        `${API_BASE}/api/admin/users/${userId}/contacts/export`,
        {
          headers: { "x-admin-key": ADMIN_KEY },
          responseType: "blob",
          timeout: 30000,
            ...
            <div className="bg-red-50 border border-red-200 rounded-lg p-6">
              ...
              <div className="space-y-2 text-left text-xs text-red-600 bg-red-100 p-3 rounded">
                ...
                <ul className="list-disc list-inside space-y-1">
                  <li>
                    Render free tier service is sleeping (takes 50-90 seconds to
                    wake up)
                  </li>
            ...
            <>
              ...
              <div className="mt-4 bg-blue-50 border border-blue-200 rounded-lg p-4 text-left text-xs text-gray-600">
                <p className="font-semibold text-blue-900 mb-2">
                  ℹ️ Why is this taking so long?
                </p>
                <p>
                  Render&apos;s free tier services automatically sleep after 15
                  minutes of inactivity to save resources. When you access the
                  dashboard, it needs to wake up the server, which can take
                  50-90 seconds on the first request.
                </p>
              </div>
            </>
          )}
      ...
      <aside className="w-60 min-h-screen bg-white border-r border-gray-200 flex flex-col shadow-sm shrink-0">
        ...
        <nav className="flex-1 px-3 py-4 space-y-1">
          {[
            ...
            <button
              ...
              className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg text-base font-medium text-gray-700 ${bg} transition-colors`}
            >
        ...
        <div className="px-3 py-4 border-t border-gray-100">
          <button
            ...
            className="w-full flex items-center gap-3 px-4 py-3 rounded-lg text-base font-medium text-red-600 hover:bg-red-100 transition-colors"
          >
            ...
            <div className="px-6 py-4 border-b border-gray-200">
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                ...
                <div className="flex gap-2">
                  <button
                    onClick={() =>
                      setCreditSort(creditSort === "desc" ? null : "desc")
                    }
                  ...
                  <button
                    onClick={() =>
                      setCreditSort(creditSort === "asc" ? null : "asc")
                    }
                    className={`flex items-center px-4 py-2 rounded-lg transition ${
                      creditSort === "asc"
                        ? "bg-blue-600 text-white hover:bg-blue-700"
                      ...
                      <tr key={user._id} className="hover:bg-gray-50">
                        ...
                        <td className="px-3 py-3 whitespace-nowrap">
                          <div className="flex gap-2">
                            ...
                            <button
                              onClick={() =>
                                deleteUser(user._id, user.name || user.phone)
                              }
